...
def visualize_loss(fig, row=1, col=4, dict=None, title=None, epoch=None):
    ...
    ax1.set_title('training loss-label')
    ...
    ax2.set_ylabel('loc loss')
    lns2 = plt.plot(dict[title[0]], dict[title[2]], 'crimson', label='Loc loss')
    lns = lns1 + lns2
    ...
    ax1.set_title('training loss-unlabel')
    ax1.set_xlabel('epoch')
    ax1.set_ylabel('rec loss')
    lns = plt.plot(dict[title[0]], dict[title[3]], 'royalblue', label='Rec loss')
    labs = [l.get_label() for l in lns]
    ...
    ax1.set_xlabel('epoch')
    ax1.set_ylabel('seg loss')
    lns1 = plt.plot(dict[title[0]], dict[title[4]], 'royalblue', label='Seg loss')
    ...
    lns2 = plt.plot(dict[title[0]], dict[title[5]], 'crimson', label='Rec loss')
    lns = lns1 + lns2
    labs = [l.get_label() for l in lns]
    if epoch == 0:
        plt.grid(which='major', axis='y', linestyle='--')
    ...
    ax1.set_xlabel('epoch')
    ax1.set_ylabel('Dice')
    lns1 = plt.plot(dict[title[0]], dict[title[6]], 'royalblue', label='Seg Dice')
    ...
    ax2.set_xlabel('epoch')
    ax2.set_ylabel('MSE')
    lns2 = plt.plot(dict[title[0]], dict[title[7]], 'crimson', label='Rec MSE')
    lns = lns1 + lns2
    labs = [l.get_label() for l in lns]
