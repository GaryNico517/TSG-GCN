...
class Flip(object):
    ...
    def __call__(self, sample):
        image, mask, hm = sample['image'], sample['mask'], sample['heat_map']
        ...
        if self.frontend:
            if f_bool:
                image, mask, hm = image[:, ::-1, :, :], mask[::-1, :, :], hm[::-1, :, :]
            else:
                image, mask, hm = image[:, :, :, :], mask[:, :, :], hm[:, :, :]
        ...
        return {'image': image.copy(), 'mask': mask.copy(), 'heat_map': hm.copy()}
...
class Crop(object):
    ...
    def __call__(self, sample):
        ...
        hm = hm[front: front + new_z, top: top + new_h, left: left + new_w]

        return {'image': image, 'mask': mask, 'heat_map':hm}
